...
export default boot(async (/* { app, router, ... } */) => {
  ...
  const siteName = store.getSetting('isite::siteName')

  if (manifestLink) {
    const startUrl = `${window.location.origin}${window.location.pathname}`;
    const name = store.getSetting('isite::manifestName') ?? store.getSetting('isite::siteName');
    const shortName = store.getSetting('isite::manifestShortName') ?? name;
    const description = store.getSetting('isite::manifestDescription') ?? store.getSetting('isite::siteDescription') ?? siteName;
